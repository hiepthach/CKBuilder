...
- **Unlock CKB**: Consume the locked cell by providing the correct plaintext password, encoded via Molecule schema, in the witness.
...
This project utilizes **Molecule Serialization** for the witness payload (`HashLockWitness`). This ensures:
- **Zero-copy deserialization** on the CKB-VM, saving gas (cycles).
- Canonical and deterministic byte structures.
- Easy extensibility (e.g., adding `message` or `signature` fields to the lock logic).
